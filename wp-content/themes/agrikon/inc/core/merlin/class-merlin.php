...
class Merlin {
    ...
    protected function server_requirements($step) {
        ...
        <table class="widefat" cellspacing="0">
            <tbody>
                ...
                <?php if ( function_exists( 'ini_get' ) ) : ?>
                    ...
                    if ( function_exists( 'phpversion' ) ) {
                        ...
                        <?php if ($php_check == 'error'): ?>
                            <tr>
                                <td><?php esc_html_e( 'PHP Version', 'agrikon' ); ?></td>
                                <td>
                                    <span class="dashicons dashicons-warning red"></span> <span><?php echo esc_html($phpversion); ?></span>
                                    <div><small><?php esc_html_e('Recommended: ', 'agrikon'); ?><strong>7.0 or higher</strong></small></div>
                                </td>
                            </tr>
                        <?php endif; ?>
                    <?php	} ?>
                    ...
                    <?php if ($max_input_vars == 'error'):
                        ...
                        ?>
                        <tr>
                            <td><?php esc_html_e( 'max_input_vars', 'agrikon' ); ?></td>
                            <td>
                                <span class="dashicons dashicons-warning <?php echo esc_attr($color); ?>"></span> <span><?php echo ini_get('max_input_vars'); ?></span>
                                <div><small><?php esc_html_e('Recommended: ', 'agrikon'); ?><strong>3000</strong></small></div>
                            </td>
                        </tr>
                    <?php endif; ?>
                    ...
                    <?php if ($memory_limit == 'error'): ?>
                        <tr>
                            <td><?php esc_html_e( 'memory_limit', 'agrikon' ); ?></td>
                            <td>
                                <span class="dashicons dashicons-warning red"></span> <span><?php echo ini_get('memory_limit'); ?></span>
                                <div><small><?php esc_html_e('Recommended: ', 'agrikon'); ?><strong>256M</strong></small></div>
                            </td>
                        </tr>
                    <?php endif; ?>
                    ...
                    <?php if ($max_execution_time == 'error'): ?>
                        <tr>
                            <td><?php esc_html_e( 'max_execution_time', 'agrikon' ); ?></td>
                            <td>
                                <span class="dashicons dashicons-warning red"></span> <span><?php echo ini_get('max_execution_time'); ?></span>
                                <div><small><?php esc_html_e('Recommended: ', 'agrikon'); ?><strong>300</strong></small></div>
                            </td>
                        </tr>
                    <?php endif; ?>
                    ...
                    <?php if ($upload_max_filesize == 'error'): ?>
                        <tr>
                            <td data-export-label="Max Upload Size"><?php esc_html_e( 'Max Upload Size', 'agrikon' ); ?>:</td>
                            <td>
                                <span class="dashicons dashicons-warning red"></span> <span><?php echo size_format( wp_max_upload_size() ); ?></span>
                                <div><small><?php esc_html_e('Recommended: ' , 'agrikon'); ?><strong>12M</strong></small></div>
                            </td>
                        </tr>
                    <?php endif; ?>
                ...
                $posting['fsockopen_curl']['help'] = '<a href="#" class="help_tip" data-tip="' . esc_attr__( 'Payment gateways can use cURL to communicate with remote servers to authorize payments, other plugins may also use it when communicating with remote services.', 'agrikon' ) . '">[?]</a>';
        ...
        <?php if ( $go == false) { ?>
            <?php if ( $step == 'welcome' ) : ?>
                <p class="server_check_notice step_welcome red"><?php echo sprintf( __( 'Your server does not match the recommended settings. You might not be able to import the full demos.', 'agrikon' ), '<span class="underline">', '</span>' ); ?></p>
                <p class="server_check_notice step_welcome"><?php echo sprintf( __( '%sAdjust your settings%s, or proceed with caution :(', 'agrikon' ), '<a href="https://ninetheme.com/docs/wordpress-server-requirements/" target="_blank">', '</a>' ); ?></p>
            <?php else: ?>
                <p class="server_check_notice step_import red"><?php echo sprintf( __( 'Your server does not match the recommended settings. %sImport the BASIC Demo%s only, or adjust your settings.', 'agrikon' ), '<span class="underline">', '</span>' ); ?></p>
            <?php endif; ?>
        <?php } ?>
    ...
    */
    protected function plugins() {
        ...
        if(!$purchase_code) {
            echo '<p>' . esc_html__('Please verify the purchase code first.', 'agrikon') . '</p>';
                echo sprintf( '<a class="relative" href="%s">%s</a>',
                esc_url( admin_url( '/' ) ),
                esc_html__('Back Admin Page', 'agrikon')
                );
            exit(0);
        }
        // Variables.
    ...
    */
    protected function content() {
        ...
        $landing_url = function_exists( 'agrikon_merlin_local_import_files' ) ? agrikon_merlin_local_import_files() : array();
    ...
    */
    public function _ajax_activate_license() {
        ...
        switch ($output->status) {
            case 200:
                add_option('envato_purchase_code_29806632', $license_key, '', 'yes');
