        ...
        private function is_minimum_version () {
            ...
            if ($this->min_redux_version != '') {
                if (version_compare($redux_ver, $this->min_redux_version) < 0) {
                    $msg = '<strong>' . esc_html__( 'The', 'agrikon') . ' ' .  $this->ext_name . ' ' .  esc_html__('extension requires', 'agrikon') . ' Redux Framework ' . esc_html__('version', 'agrikon') . ' ' . $this->min_redux_version . ' ' .  esc_html__('or higher.','agrikon' ) . '</strong>&nbsp;&nbsp;' . esc_html__( 'You are currently running', 'agrikon') . ' Redux Framework ' . esc_html__('version','agrikon' ) . ' ' . $redux_ver . '.<br/><br/>' . esc_html__('This field will not render in your option panel, and features of this extension will not be available until the latest version of','agrikon' ) . ' Redux Framework ' . esc_html__('has been installed.','agrikon' );
                    ...
                    if (method_exists('Redux_Admin_Notices', 'set_notice')) {
                        ...
                        echo     '<p>';
                        echo agrikon_sanitize_data($msg);
                        echo     '</p>';
                        echo '</div>';
                    }
