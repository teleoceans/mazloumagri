...
if ( ! function_exists( 'agrikon_breadcrumbs' ) ) {
    ...
    function agrikon_breadcrumbs( $args = array() ) {

        $breadcrumb = apply_filters( 'breadcrumb_trail_object', null, $args );

        if ( ! is_object( $breadcrumb ) )
        $breadcrumb = new Agrikon_Breadcrumb( $args );
...
class Agrikon_Breadcrumb {
    ...
    */
    public function trail() {
        ...
        if ( 0 < $item_count ) {
            ...
            $breadcrumb .= sprintf(
                '<%s class="breadcrumbs thm-breadcrumb list-unstyled">',
            ...
            foreach ( $this->items as $item ) {
                ...
                $seperator = '';
                // Build the seperator.
                if ( $item_count > 0 && $item_count !== $item_position ) {
                    $seperator = '<'.tag_escape( $this->args['item_tag'] ).' class="breadcrumb_link_seperator"> / </'.tag_escape( $this->args['item_tag'] ).'>';
                }

                // Build the list item.
                $breadcrumb .= sprintf( '<%1$s %2$s>%3$s%4$s</%1$s>%5$s', tag_escape( $this->args['item_tag'] ),$attributes, $item, $meta, $seperator );
            }
    ...
    */
    protected function set_labels() {
        ...
        $defaults = array(
            'browse'              => esc_html__( 'Browse:',                               'agrikon' ),
            'aria_label'          => esc_attr_x( 'Breadcrumbs', 'breadcrumbs aria label', 'agrikon' ),
            'home'                => esc_html__( 'Home',                                  'agrikon' ),
            'error_404'           => esc_html__( '404 Not Found',                         'agrikon' ),
            'archives'            => esc_html__( 'Archives',                              'agrikon' ),
            // Translators: %s is the search query.
            'search'              => esc_html__( 'Search results for: %s',                'agrikon' ),
            // Translators: %s is the page number.
            'paged'               => esc_html__( 'Page %s',                               'agrikon' ),
            // Translators: %s is the page number.
            'paged_comments'      => esc_html__( 'Comment Page %s',                       'agrikon' ),
            // Translators: Minute archive title. %s is the minute time format.
            'archive_minute'      => esc_html__( 'Minute %s',                             'agrikon' ),
            // Translators: Weekly archive title. %s is the week date format.
            'archive_week'        => esc_html__( 'Week %s',                               'agrikon' ),
    ...
    */
    protected function add_singular_items() {
        ...
        if ( $post_title = single_post_title( '', false ) ) {
            ...
            elseif ( true === $this->args['show_title'] )
            $this->items[] = apply_filters( 'breadcrumb_trail_singular_alt_title', $post_title );
        }
    }
    ...
    */
    protected function add_minute_hour_archive_items() {
        ...
        $this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'agrikon' ) ) );
    }
    ...
    */
    protected function add_minute_archive_items() {
        ...
        // Add the minute item.
        if ( true === $this->args['show_title'] )
        $this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'agrikon' ) ) );
    }
    ...
    */
    protected function add_hour_archive_items() {
        ...
        // Add the hour item.
        if ( true === $this->args['show_title'] )
        $this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'agrikon' ) ) );
    }
    ...
    */
    protected function add_day_archive_items() {
        ...
        $year  = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'agrikon' ) ) );
        $month = sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'agrikon' ) ) );
        $day   = sprintf( $this->labels['archive_day'],   get_the_time( esc_html_x( 'j', 'daily archives date format',   'agrikon' ) ) );
    ...
    */
    protected function add_week_archive_items() {
        ...
        $year = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'agrikon' ) ) );
        $week = sprintf( $this->labels['archive_week'],  get_the_time( esc_html_x( 'W', 'weekly archives date format', 'agrikon' ) ) );
    ...
    protected function map_rewrite_tags( $post_id, $path ) {
        ...
        if ( is_array( $matches ) ) {
            ...
            foreach ( $matches as $match ) {
                ...
                if ( '%year%' == $tag )
                $this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_year_link( get_the_time( 'Y', $post_id ) ) ), sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'agrikon' ) ) ) );

                // If using the %monthnum% tag, add a link to the monthly archive.
                elseif ( '%monthnum%' == $tag && true === $this->args['show_month'] )
                $this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) ), sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'agrikon' ) ) ) );

                // If using the %day% tag, add a link to the daily archive.
                elseif ( '%day%' == $tag && true === $this->args['show_day'] )
                $this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ), sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'agrikon' ) ) ) );
